class BuildingsController < ApplicationController
  before_action :set_building, only: %i[ show edit update destroy reject_request accept_request ]

  # GET /buildings or /buildings.json
  def index
    @buildings = Building.registered
  end
  ...
  def reject_request
    @building.update!(register_status: false)

    respond_to do |format|
      format.html { redirect_to @building, notice: "Building registration rejected." }
      format.json { render :show, status: :ok, location: @building }
    end
  end
  ...
  private
    ...
    def building_params
      params.expect(building: [ :building_name, :address, :location, :other_description, :register_status ])
    end
end
